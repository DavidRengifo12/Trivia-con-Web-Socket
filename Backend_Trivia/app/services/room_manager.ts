...
import type { Room, Player, RoomCode } from '../../app/Types/Ws.js'
...
export default class RoomManager {
  ...
  bindSocket(socket: Socket) {
    // Moderador crea sala
    socket.on('moderator:create_room', (_, ack?: Function) => {
    ...
    // Jugador se une
    socket.on('player:join_room', ({ code, nickname }, ack?: Function) => {
    ...
    // Responder
    socket.on('player:answer', ({ code, answer }, ack?: Function) => {
      const room = this.rooms.get(code)
      ...
      const isCorrect = answer.trim().toLowerCase() === room.currentRound.question.correctAnswer.trim().toLowerCase()

      room.currentRound.answers.set(player.id, { playerId: player.id, answer, timeMs, isCorrect })
      ack?.({ ok: true })
    })
  ...
  private emitRoomState(code: RoomCode) {
    ...
    this.io.to(code).emit('room:state', { code, players })
  }
}*/

import { Server as IOServer, Socket } from 'socket.io'
import { randomUUID } from 'node:crypto'
import type { Room, Player, RoomCode } from '../../app/services/room_manager.js'
